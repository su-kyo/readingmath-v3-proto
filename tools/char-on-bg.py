...
찍어 주는 숫자
    · 자체 대비  캐릭터 안에서 가장 밝은 면 ↔ 가장 어두운 면의 대비비.
                 이 화면에서 실제로 일하는 숫자다. 확정 기준안(초3-1 로봇)이 11.09다.
                 8 이상이면 합격권, 5 미만이면 구조가 뭉갠 것이니 다시 본다.
    · 몸 대비    캐릭터 평균 밝기 ↔ 그 자리 배경 평균 밝기의 대비비. **참고값이다.**
                 게이트로 쓰지 않는다 — 기준안도 1.14·1.20이라 그 숫자로 자르면
                 사용자가 합격시킨 안까지 떨어진다. 1.0 근처면 밝은 면을 더 밝게 한다.

    숫자는 보조다. 판정은 검사 이미지를 눈으로 보고 한다. 다섯 가지를 본다.
      ...
      ⑤ 기준안과 나란히 놓았을 때 형제로 보이는가
    ②와 ③이 1차 시안들의 실제 사망 원인이다. 대비보다 먼저 본다.

전제
    · 캐릭터 원본은 1:1 정사각, 배경이 단색으로 꽉 차 있다(기본값 마젠타 #FF00FF).
      이미 투명 PNG면 --keep 을 붙인다.
    · 오브젝트를 전부 「완료(실물)」로 깔기 때문에 **최악 조건이다.** 실제 화면에서는
      미완료 단원이 홀로그램(무채색)이라 컬러 캐릭터가 저절로 튄다. 여기서 아슬아슬해도
      실제 화면에서는 충분할 수 있다 — 그래서 숫자로 자르지 않는다.
"""
...
def verdict(body, self_):
    """확정 기준안(초3-1)이 몸 대비 1.14·자체 대비 11.09로 「합격권」에 들어오게 잡았다.
       몸 대비는 아주 낮을 때만(1.05 미만) 발목을 잡는다."""
    if self_ >= 8.0 and body >= 1.05: return '합격권'
    if self_ >= 5.0:                  return '눈으로 판단'
    return '구조 의심'
...
def main():
    ...
    print('  ' + '-' * 52)
    worst = '합격권'
    order = {'합격권': 0, '눈으로 판단': 1, '구조 의심': 2}
    for label, body, self_ in results:
        v = verdict(body, self_)
        if order[v] > order[worst]:
            worst = v
        print('  %-16s  %6.2f       %5.2f        %s' % (label, self_, body, v))
    print('  ' + '-' * 52)
    print('  최종 = 더 나쁜 쪽:  %s' % worst)
    print('  참고 — 확정 기준안(초3-1 로봇)은 자체 대비 11.09 · 몸 대비 1.14/1.20 이다.')
    print('\n  숫자가 「합격권」이어도 검사 이미지를 눈으로 보고 다섯 가지를 판정한다.')
    print('    ① 가장 먼저 눈이 가는 것이 캐릭터인가   ② 머리·몸통·팔·다리가 하나하나 보이는가')
    print('    ③ 몇 색인지 세지 않고 알 수 있는가      ④ 얼굴이 밝은 면 위에 있는가')
    print('    ⑤ 기준안과 형제로 보이는가')
    print('  ②와 ③이 1차 시안들의 실제 사망 원인이다. 대비보다 먼저 본다.')
    print('  오브젝트를 전부 실물로 깐 최악 조건이다 — 실제 화면은 홀로그램이라 더 유리하다.')
