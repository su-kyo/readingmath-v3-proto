...
> ⚠️ **2026-08-17 확인 — 색값은 이 문서가 아니라 `styles/tokens.css`가 현행이다.**
>
> 이 문서는 2026-07-14 이후 갱신되지 않았고, 그 뒤 `tokens.css`가 여러 번 바뀌었다.
> 중립색 램프(n-050 이하)부터 값이 서로 다르다. 아래 「단일 정본」 선언은 **원칙과
> 구조에 대해서만** 유효하다.
>
> - **원칙·구조**(6원칙, 3단 토큰, 슬롯 계약, 금지 규칙) → **이 문서가 기준**
> - **실제 색값** → **`styles/tokens.css`가 기준**
> - `design-tokens.json`은 2026-07-14 스냅샷이며 색값이 낡았다
>
> **2026-08-17 대비 감사 반영** — 전 화면을 WCAG AA로 맞추면서 아래가 바뀌었다.
> 자세한 내용과 수치는 `docs/color-contrast-audit.md`.
>
> - 새 단계 `--orange-450`(주 CTA 표면) · `--n-350`(다크 meta 글자)
> - 새 슬롯 **`--th-solid`** — 흰 글자가 올라가는 칠면은 반드시 이것을 쓴다.
>   `--th-accent`(500)는 흰 글자와 4.1~4.4로 미달이므로 글자 받침으로 쓰지 않는다.
> - GNB 모드색 4종이 500/브랜드값 → 600단계로
> - 에셋 연동색·배경 아트색이 토큰으로 승격(`--asset-*`, `--art-*`).
>   화면에서 hex를 직접 쓰는 곳은 이제 0곳이다.
...
### 3.0b 칠면·글자 정책 — "대비 잣대는 하나가 아니다" (v1.6 ★)

Phase 2 대비 감사의 오류를 바로잡는 정책이다. 당시 **모든 칠면에 "작은 흰 글자
4.5:1" 한 가지 잣대**를 적용해 CTA·모드색·섹션 칠면이 전부 어두운 쪽으로 밀렸고
("칙칙" 판정), 브랜드 비비드는 장식 자리로 쫓겨났다. WCAG에는 세 가지 잣대가 있다:

| 부류 | 무엇이 올라가나 | 기준 | 쓰는 스텝 |
|---|---|---|---|
| **A. 텍스트 칠면** | 작은 글자 (<19px) | 글자↔칠면 **4.5:1** | 600 + 흰 글자, 또는 tint + ink |
| **B. 정체성 칠면** | 크고 굵은 라벨만 (**≥19px w700**) | 글자↔칠면 **3:1** (큰 텍스트) | **450 IDENTITY** + 흰 라벨 |
| **C. 장식면** | 글자 없음 (아이콘 도형·마커·일러스트) | 면↔주변 배경 **3:1** | 400 VIVID |

- **450 IDENTITY 스텝(신설)** — "흰 굵은 글자 3:1을 정확히 만족하는 가장 밝은 색"을
  가족마다 `solve_identity()`로 푼 값. L 고정이 아니라 **대비로 푸는** 이유: 같은 L이라도
  hue마다 휘도가 달라(노랑↑ 파랑↓) 흰 글자 대비가 널뛰기 때문.
- 적용처: 주 CTA(`--primary`) · GNB 모드탭(`--mode-*`) · 채점/확인 버튼(`--th-vivid`) ·
  상태 강조(`--ok/no/info-vivid`).
- **하드 룰**: 450 위에 19px 미만 글자를 얹으면 위반이다. 작은 글자가 필요해지면
  그 칠면은 정체성 칠면이 아니다 — A부류(600 또는 tint+ink)로 바꾼다.
- 밝은 가족 예외(앰버처럼 흰 글자가 물리적으로 불가한 경우): **400 칠 + n-900 잉크 글자**
  (전 가족 4.5 이상 검증됨 — 생성기 ③번 항목).
...
### 3.4b 주요 CTA (오렌지 — 전 섹션 공통) — v1.6 개정

| 토큰 | 값 | 비고 |
|---|---|---|
| `--primary` | **orange-450 `#E7730E`** | IDENTITY 스텝 — 흰 굵은 글자 정확히 3.06:1 |
| `--primary-hover` | orange-400 `#FE811B` | 호버 = 더 밝게 (살아나는 방향) |
| `--primary-pressed` | orange-500 `#C3600A` | 눌림 = 더 깊게 |
| `--primary-deep` | orange-700 `#6E3403` | 드롭 섀도 shade |

- **라벨 규칙 (§3.0b-B)**: CTA 라벨은 **19px 이상 + w700 이상 필수** (WCAG 큰 텍스트 3:1).
  작은 텍스트를 얹어야 하면 그 버튼은 CTA가 아니다 — `--th-solid`(600) 또는 tint+ink.
- **32배경 생존 규칙**: CTA는 행성 홈 32종 배경 어디서든 같은 주황 하나다. 배경에 맞춰
  물들이지 않는다. 검증은 특정 배경과의 수치가 아니라 **`tools/cta-check.html`에서
  32개 배경 전부에 대해 버튼 주변 면 대비 ≥3:1**로 판정한다.
- 흰 글자 4.5를 CTA에 강요했던 Phase 2의 `#B75301`은 폐기 (칙칙 판정 — v1.6의 발단).
...
*v1.6 — 2026-08-17. **칠면·글자 정책 신설 + IDENTITY 스텝(450) 도입** (사용자: "대비는 통과하는데
화면 전체가 칙칙 — 흰 글자 4.5를 모든 칠면에 강요한 게 원인"). §3.0b 칠면 3부류 정책
(A 텍스트 칠면 4.5 / B 정체성 칠면 3.0 + ≥19px w700 라벨 / C 장식면 면대비 3.0) 신설.
전 가족에 450 IDENTITY 스텝 생성(`solve_identity()` — 흰 굵은 글자 3:1을 정확히 만족하는 최대 밝기).
`--primary`=orange-450 `#E7730E`(구 #B75301 폐기) · `--mode-*` 600→450 · `--th-vivid` 슬롯 신설
(채점/확인 버튼) · 상태색 vivid 변형 추가 · CTA/모드탭/채점 라벨 19px 통일.
**다크 틴트 분리**: 다크에서 `--th-tint(-strong)` 8/15%→24/38% (끊어읽기·빈칸 채움이 배경과
안 갈라지던 원인). 형광펜 `--art-mark-*` 수치 재도출(면 2.96 · 글자 5.04). 검증도 역할별로 재정의:
생성기 6항목 + `tools/cta-check.html`(CTA가 홈 32배경 전부에서 면대비 3:1 생존 판정).*
